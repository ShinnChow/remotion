...
## Example: Boolean values<AvailableFrom v="4.0.530" />

Use `Easing.step1` to switch a boolean at a keyframe. The previous value is held until the next keyframe.

```tsx twoslash title="MyComposition.tsx"
import {Easing, interpolate, useCurrentFrame} from 'remotion';

const frame = useCurrentFrame();
const disabled = interpolate(frame, [0, 60], [false, true], {
  easing: Easing.step1,
});
```

All output values must be booleans, and every segment must use `Easing.step1`.
