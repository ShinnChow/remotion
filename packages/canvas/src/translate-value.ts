...
// Drops floating point noise such as 10.020000000000001.
const normalizeNumber = (value: number): number =>
	roundToDecimalPlaces(value, 6);
...
export const parseTranslateWithUnits = (
	...
	const x = normalizeNumber(Number(match[1]));
	const y = match[3] === undefined ? 0 : normalizeNumber(Number(match[3]));
	const z = match[5] === undefined ? null : normalizeNumber(Number(match[5]));
...
/** Pixel translate coordinates; percentages and unparseable values read as the origin. */
...
const formatTranslateCoordinate = (
	...
	const rounded = roundToDecimalPlaces(normalizeNumber(value), decimalPlaces);
